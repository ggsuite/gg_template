...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_template.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggTemplate = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_template.dart',
      );

      expect(
        ggTemplate,
        isNotNull,
        reason: 'Item for executing bin/gg_template.dart not found',
      );

      expect(ggTemplate['name'], 'gg_template.dart');
      expect(ggTemplate['type'], 'dart');
      expect(ggTemplate['request'], 'launch');
